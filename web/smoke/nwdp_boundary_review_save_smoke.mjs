...
await page.getByRole("button", { name: "Save review metadata" }).click();

try {
  await page.getByText("Candidate remains inactive and unpromoted").waitFor({ timeout: 15000 });
} catch (error) {
  const bodyText = await page.locator("body").innerText().catch(() => "");
  await page.screenshot({
    path: `${screenshotDir}/nwdp-boundary-review-save-failed.png`,
    fullPage: true,
  });
  throw new Error(`Save smoke did not see success message. Browser events: ${JSON.stringify(browserEvents, null, 2)} Page text excerpt: ${bodyText.slice(0, 5000)}`);
}
await page.getByText("Runtime matching: disabled").waitFor({ timeout: 15000 });
...
await page.screenshot({
  path: `${screenshotDir}/nwdp-boundary-review-save.png`,
  fullPage: true,
});
...
await page.getByRole("heading", { name: "Review history" }).waitFor({ timeout: 15000 });
...
console.log(JSON.stringify({
  schema_version: "nwdp_boundary_review_save_web_smoke.v1",
  status: "PASSED",
  url: reviewUrl,
  screenshot: path.join(screenshotDir, "nwdp-boundary-review-save.png"),
  tenant_id: tenantId,
  rows_seen: rows,
