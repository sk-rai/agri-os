...
import {
  ...
  type ProductCatalogAuditEventDto,
...
export default function ProductsPage() {
  ...
  const [auditEvents, setAuditEvents] = useState<ProductCatalogAuditEventDto[]>([]);
  const [auditFilter, setAuditFilter] = useState({ entity_type: "", entity_code: "" });
  ...
  const [product, setProduct] = useState({ code: "", canonical_input_code: "", manufacturer_code: "", brand_name: "", composition: "", registration_number: "", registration_authority: "", source_url: "", source_notes: "", source_text: "", sku: "", quantity: "", unit: "kg", pack_label: "", barcode: "", reason: "Created in admin" });
  ...
  const loadAudit = async () => {
    try {
      const data = await productCatalogApi.audit({ entityType: auditFilter.entity_type || undefined, entityCode: auditFilter.entity_code || undefined, limit: 50 });
  ...
  const createMfg = async () => { if (!canEditCatalog) { setError("Your current role can view products but cannot edit the product catalog."); return; } setBusy(true); setError(null); try { await productCatalogApi.createManufacturer({ ...mfg, aliases: [] }); setMfg({ ...mfg, code: "", canonical_name: "", short_name: "" }); setNotice("Manufacturer created"); await load(); await loadAudit(); } catch (e) { setError(e); } finally { setBusy(false); } };
  const createProduct = async () => { if (!canEditCatalog) { setError("Your current role can view products but cannot edit the product catalog."); return; } setBusy(true); setError(null); try { await productCatalogApi.createProduct({ code: product.code, canonical_input_code: product.canonical_input_code, manufacturer_code: product.manufacturer_code, brand_name: product.brand_name, composition: product.composition || null, registration_number: product.registration_number || null, registration_authority: product.registration_authority || null, source_url: product.source_url || null, source_notes: product.source_notes || null, source_text: product.source_text || null, country: "India", packages: [{ sku: product.sku, quantity: product.quantity, unit: product.unit, pack_label: product.pack_label, barcode: product.barcode || null }], reason: product.reason }); setNotice("Product and package created"); await load(); await loadAudit(); } catch (e) { setError(e); } finally { setBusy(false); } };
  const approve = async () => { if (!canEditProjectApprovals) { setError("Your current role can view products but cannot change project product approvals."); return; } setBusy(true); setError(null); try { await productCatalogApi.approveProduct(approval.project_id, approval.product_code, { enabled: approval.enabled, preferred: approval.preferred, display_order: Number(approval.display_order), reason: approval.reason }); setNotice("Project approval saved"); await loadAudit(); } catch (e) { setError(e); } finally { setBusy(false); } };
  ...
  const applyProductCsvImport = async (batch: ProductCsvImportBatch, reason: string) => {
    ...
    try { const applied = await productCatalogApi.applyCsv(batch.batch_id, reason); setCsvReport(applied.report); setNotice("Product CSV import applied"); await Promise.all([load(), loadProductImportHistory(), loadAudit()]); }
    catch (e) { setError(e); }
    finally { setCsvBusy(false); }
  };
  ...
  return <div>
    ...
    <ProductCatalogAuditPanel events={auditEvents} filter={auditFilter} onFilter={setAuditFilter} onRefresh={loadAudit} />
    <div className="mt-6 grid gap-6 xl:grid-cols-2"><DataTable title="Branded products" headers={["Code", "Brand", "Canonical input", "Manufacturer", "Packages", "Source info", "Status"]}>{products.map(x => <tr key={x.code}><td className="p-3 font-mono text-xs">{x.code}</td><td className="p-3 font-medium">{x.brand_name}</td><td className="p-3">{x.canonical_input_code}</td><td className="p-3">{x.manufacturer_name}</td><td className="p-3">{x.packages.map(p => p.pack_label).join(", ")}</td><td className="p-3 min-w-72"><ProductSourceInfo product={x} /></td><td className="p-3"><button disabled={!canEditCatalog} title={canEditCatalog ? undefined : "Your role cannot edit the product catalog."} onClick={async () => { if (!canEditCatalog) return; await productCatalogApi.updateProduct(x.code, { status: x.status === "ACTIVE" ? "DISCONTINUED" : "ACTIVE", reason: "Admin status change" }); await load(); await loadAudit(); }} className="rounded border px-2 py-1 text-xs disabled:opacity-50">{x.status}</button></td></tr>)}</DataTable><div><div className="mb-3 grid grid-cols-4 gap-2"><Field label="Rule crop" value={ruleFilter.crop_code} set={v => setRuleFilter({ ...ruleFilter, crop_code: v })} /><Field label="Rule stage" value={ruleFilter.stage_code} set={v => setRuleFilter({ ...ruleFilter, stage_code: v })} /><Field label="Rule activity" value={ruleFilter.activity_type} set={v => setRuleFilter({ ...ruleFilter, activity_type: v })} /><button onClick={loadRules} className="mt-5 rounded border px-3 py-2 text-sm">Refresh</button></div><DataTable title="Dosage rules" headers={["Scope", "Crop/stage", "Input", "Dosage", "Enabled"]}>{rules.map(r => <tr key={r.id}><td className="p-3">{r.rule_scope}</td><td className="p-3"><div>{r.crop_code} · {r.stage_code}</div><div className="text-xs text-gray-500">{r.activity_type}</div></td><td className="p-3"><div className="font-medium">{r.input_name}</div><div className="font-mono text-xs">{r.input_code}</div></td><td className="p-3">{r.dosage.quantity || "-"} {r.dosage.unit || ""}/{r.dosage.area_unit}</td><td className="p-3"><button disabled={!canEditCatalog} title={canEditCatalog ? undefined : "Your role cannot edit input dosage rules."} onClick={() => toggleRule(r)} className="rounded border px-2 py-1 text-xs disabled:opacity-50">{r.enabled ? "Enabled" : "Disabled"}</button></td></tr>)}</DataTable></div></div>
  </div>;
}
function ProductCatalogAuditPanel({ events, filter, onFilter, onRefresh }: { events: ProductCatalogAuditEventDto[]; filter: { entity_type: string; entity_code: string }; onFilter: (value: { entity_type: string; entity_code: string }) => void; onRefresh: () => void }) {
  const entityTypes = ["", "MANUFACTURER", "PRODUCT", "PACKAGE", "PROJECT_PRODUCT_APPROVAL"];
  const importActions = new Set(["IMPORT_CREATE_MANUFACTURER", "IMPORT_UPDATE_MANUFACTURER", "IMPORT_CREATE_PRODUCT", "IMPORT_UPDATE_PRODUCT", "IMPORT_CREATE_PACKAGE", "IMPORT_UPDATE_PACKAGE"]);
  return <section className="mt-6 rounded bg-white p-5 shadow">
    <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
      <div>
        <h2 className="font-semibold">Product catalog audit trail</h2>
        <p className="mt-1 text-xs text-gray-500">Trace manual edits and CSV imports across manufacturers, branded products, packages, and project approvals.</p>
      </div>
      <button onClick={onRefresh} className="rounded border px-3 py-2 text-xs font-semibold text-gray-700 hover:bg-gray-50">Refresh audit</button>
    </div>
    <div className="mt-4 grid gap-3 md:grid-cols-3">
      <label className="text-xs text-gray-500">Entity type<select value={filter.entity_type} onChange={(event) => onFilter({ ...filter, entity_type: event.target.value })} className="mt-1 w-full rounded border p-2 text-sm text-gray-900">{entityTypes.map((type) => <option key={type} value={type}>{type || "All"}</option>)}</select></label>
      <Field label="Entity code" value={filter.entity_code} set={(value) => onFilter({ ...filter, entity_code: value })} />
      <button onClick={onRefresh} className="mt-5 rounded bg-gray-900 px-3 py-2 text-sm font-semibold text-white">Apply filters</button>
    </div>
    <div className="mt-4 overflow-x-auto">
      <table className="min-w-full text-left text-xs">
        <thead className="text-gray-500"><tr><th className="px-2 py-2">When</th><th className="px-2 py-2">Entity</th><th className="px-2 py-2">Action</th><th className="px-2 py-2">Reason</th><th className="px-2 py-2">Actor</th></tr></thead>
        <tbody>
          {events.length ? events.map((event) => <tr key={event.id} className="border-t">
            <td className="px-2 py-2 whitespace-nowrap">{new Date(event.created_at).toLocaleString()}</td>
            <td className="px-2 py-2"><div className="font-mono font-semibold">{event.entity_code}</div><div className="text-gray-500">{event.entity_type}</div></td>
            <td className="px-2 py-2"><span className={`rounded px-2 py-1 font-semibold ${importActions.has(event.action) ? "bg-blue-50 text-blue-700" : "bg-gray-50 text-gray-700"}`}>{event.action}</span></td>
            <td className="px-2 py-2">{event.reason || "-"}</td>
            <td className="px-2 py-2 font-mono text-[11px] text-gray-500">{event.actor_id || "-"}</td>
          </tr>) : <tr><td colSpan={5} className="px-2 py-6 text-center text-gray-400">No audit events found for this filter.</td></tr>}
        </tbody>
      </table>
    </div>
  </section>;
}
function ProductCsvLifecycleGuide() {
  const steps = ["Download template or export current catalog", "Edit manufacturers, branded products, and package SKUs", "Validate CSV and fix errors/warnings", "Apply only VALIDATED batches", "Review apply summary and audit trail"];
  const statuses = [
    ["VALIDATED", "Ready to apply before expiry"],
    ["INVALID", "Fix CSV and upload again"],
    ["APPLIED", "Already imported; cannot be applied again"],
    ["EXPIRED", "Validate again before applying"],
    ["STALE", "Catalog changed after validation; revalidate"],
  ];
  return <div className="mt-4 grid gap-3 rounded border border-blue-100 bg-white/70 p-3 text-xs text-blue-950 md:grid-cols-2">
    <div>
      <p className="font-semibold">Operator checklist</p>
      <ol className="mt-2 list-decimal space-y-1 pl-4">{steps.map((step) => <li key={step}>{step}</li>)}</ol>
      <p className="mt-2 text-blue-800">Products must map to a published canonical input first; use Input Catalog if a required input is missing.</p>
    </div>
    <div>
      <p className="font-semibold">Batch status meaning</p>
      <div className="mt-2 grid gap-1">{statuses.map(([status, meaning]) => <div key={status} className="flex gap-2"><span className="w-20 shrink-0 font-mono font-semibold">{status}</span><span>{meaning}</span></div>)}</div>
    </div>
  </div>;
}
...
function TextareaField({ label, value, set, rows = 3 }: { label: string; value: string; set: (v: string) => void; rows?: number }) { return <label className="block text-xs text-gray-500">{label}<textarea value={value} rows={rows} onChange={e => set(e.target.value)} className="mt-1 w-full rounded border p-2 text-sm text-gray-900" /></label>; }
function ProductSourceInfo({ product }: { product: AgriculturalProductDto }) {
  const sourceText = product.source_text || "";
  const sourceNotes = product.source_notes || "";
  return <div className="space-y-2 text-xs">
    {product.source_url ? <a href={product.source_url} target="_blank" rel="noreferrer" className="font-semibold text-blue-700 underline">Open source</a> : <span className="text-gray-400">No source URL</span>}
    {product.source_capture_status ? <span className="ml-2 rounded bg-amber-50 px-2 py-1 font-semibold text-amber-700">{product.source_capture_status}</span> : null}
    {sourceNotes ? <textarea readOnly value={sourceNotes} rows={3} className="w-full rounded border bg-gray-50 p-2 text-gray-700" /> : null}
    {sourceText ? <details><summary className="cursor-pointer text-gray-600">View captured source text</summary><textarea readOnly value={sourceText} rows={6} className="mt-1 w-full rounded border bg-gray-50 p-2 text-gray-700" /></details> : null}
  </div>;
}
