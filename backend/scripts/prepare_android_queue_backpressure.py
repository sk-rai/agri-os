"""Prepare baseline for Android sync queue pagination/backpressure QA.

Flow 27 uses many offline crop_activity CREATE events under the existing active
Android dynamic Rice/NURSERY cycle. The prep records baseline activity/finance
state and writes a canonical 25-event sample manifest Android may mirror.
"""
...
def delete_prior_indexed_rows(db, stage_id: uuid.UUID, count: int, dry_run: bool) -> dict:
    ...
    event_ids = [
        uuid.uuid5(uuid.NAMESPACE_URL, f"farmint-flow27-event-{index}")
...
def sample_event(index: int, amount: Decimal) -> dict:
    event_id = uuid.uuid5(uuid.NAMESPACE_URL, f"farmint-flow27-event-{index}")
    activity_id = uuid.uuid5(uuid.NAMESPACE_URL, f"farmint-flow27-activity-{index}")
    return {
        ...
        "metadata": {
            "source": SOURCE,
            "queue_backpressure_index": index,
            "queue_backpressure_count": DEFAULT_COUNT,
        },
    }
...
def main() -> int:
    ...
    parser.add_argument("--reset-indexed", action="store_true", help="Delete prior activities with the canonical Flow 27 notes before baselining.")
    ...
    try:
        ...
        baseline = {
            "schema_version": "android_queue_backpressure_baseline.v1",
        ...
        manifest = {
            "schema_version": "android_queue_backpressure_sample_events.v1",
            "tenant_id": TENANT_ID,
        ...
        result = {
            "schema_version": "android_queue_backpressure_prepare.v1",
            ...
            "android_payload_notes": {
                ...
                "stable_index_field": "metadata.queue_backpressure_index and notes suffix 01..N",
            },
        }
