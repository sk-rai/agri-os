...
def main() -> int:
    ...
    check(OUTPUT.exists(), "Inactive staging planner writes output", proc.stdout)
    data = json.loads(OUTPUT.read_text(encoding="utf-8"))

    check(proc.returncode == 0, "Inactive staging planner exits zero", data)
    check(data["schema_version"] == "nwdp_boundary_all_state_inactive_staging_import_plan.v1", "Planner schema version is stable", data)
    check(data["mode"] == "READ_ONLY_INACTIVE_STAGING_IMPORT_PLAN", "Planner is read-only", data)
    ...
    check(data["apply_policy"]["apply_implemented"] is False, "Planner keeps apply unimplemented", data["apply_policy"])
    check(data["apply_policy"]["apply_requires_separate_checkpoint"] is True, "Planner requires separate apply checkpoint", data["apply_policy"])
    check(data["readiness"]["ready_for_inactive_staging_import_design"] is True, "Planner is ready for import design", data["readiness"])
    check(data["readiness"]["ready_for_inactive_staging_import_apply"] is False, "Planner is not ready for staging apply", data["readiness"])
    check(data["readiness"]["ready_for_runtime_table_write"] is False, "Planner is not ready for runtime writes", data["readiness"])
