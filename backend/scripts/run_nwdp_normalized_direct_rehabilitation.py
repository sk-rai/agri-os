#!/usr/bin/env python3
"""Fail-closed normalized direct-village rehabilitation engine.
...
DEFAULT_PROPOSAL = (
    CAMPAIGN_DIR
    / "normalized_direct_rehabilitation_proposal.json"
)
DEFAULT_AUTHORIZATION = (
    CAMPAIGN_DIR
    / "normalized_direct_rehabilitation_authorization.json"
)
DEFAULT_CHECKPOINT = (
    CAMPAIGN_DIR
    / "normalized_direct_rehabilitation_checkpoint.json"
)
DEFAULT_OUTPUT = (
    CAMPAIGN_DIR
    / "normalized_direct_rehabilitation_engine_report.json"
)

PROPOSAL_CHECKSUM = (
    "11bc682d32ea23c72403cbe374bc615e3"
    "8fb6e1652a271e10692455dc8110691"
)
AUTHORIZATION_CHECKSUM = (
    "825fa696b79d24d28d86fa68e66c300b"
    "0a7c6bd0c6622af236fa422dddb117cb"
)
NATIONAL_MANIFEST_SHA256 = (
    "da132eb9cbf0446638d1ef060c866fc9"
    "313e02c5a934842b06f4929cb44101f5"
)
MANIFEST_CHECKSUM = (
    "bf7aad382b660d5af1d91ac62d618ce1"
    "828b84e74ed4cc6abdb0d789c2ee51cc"
)
MANIFEST_FILE_SHA256 = (
    "050562fd5bd15928c567cbce5913ea370"
    "011bdd4615a010f9d4652357fc87984"
)
INITIAL_CHECKPOINT_CHECKSUM = (
    "0c28545658a25b8490ae7415660b15525"
    "e2a373604343841c44504c92476ae82"
)
...
MARKER = "normalized_direct_village_rehabilitation"
SCHEMA_VERSION = (
    "nwdp_normalized_direct_rehabilitation_state_engine.v1"
)

REVIEWER_ID = "normalized-direct-village-rehabilitation"
REVIEWER_NOTES = (
    "Checksum-pinned normalized direct-village identity approved for "
    "inactive runtime staging; candidate remains inactive and unpromoted."
)
...
EXPECTED_PERMISSIONS = {
    ...
    "lookup_change_allowed": False,
    "project_match_write_allowed": False,
    "review_metadata_write_allowed": True,
    "runtime_activation_allowed": False,
    "runtime_eligibility_write_allowed": True,
    "runtime_set_identity_change_allowed": False,
    "source_file_write_allowed": False,
    "source_row_write_allowed": False,
}
...
def arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--enable-normalized-direct-rehabilitation-write",
        action="store_true",
    )
    parser.add_argument(
...
def validate_mutation_gates(
    ...
    required = {
        "enable":
            options.enable_normalized_direct_rehabilitation_write,
...
def validate_artifacts(
    ...
    checks = {
        "manifest_schema":
            ...
            == (
                "nwdp_normalized_direct_"
                "rehabilitation_manifest.v1"
            ),
        "manifest_healthy":
            manifest.get("healthy") is True,
        "manifest_read_only":
            manifest.get("database_writes_attempted")
            is False,
        "manifest_file_sha256":
        ...
        "manifest_checksum":
            ...
            == authorization.get("manifest_checksum")
        ...
        "proposal_manifest_file":
            proposal.get("manifest_file_sha256")
            == MANIFEST_FILE_SHA256,
        "authorization_schema":
            authorization.get("schema_version")
            == (
                "nwdp_normalized_direct_"
                "rehabilitation_authorization.v1"
            ),
        "authorization_status":
            authorization.get("authorization_status")
            == "AUTHORIZED",
        "authorization_count":
            authorization.get("authorization_count")
            == 1,
        "authorization_checksum":
        ...
        "authorization_proposal":
            authorization.get("proposal_checksum")
            == PROPOSAL_CHECKSUM,
        "permissions_exact":
        ...
        "runtime_set":
            ...
            == authorization.get("runtime_set_id")
        ...
        "checkpoint_schema":
            ...
            == (
                "nwdp_normalized_direct_"
                "rehabilitation_checkpoint.v1"
            ),
        "checkpoint_proposal":
            checkpoint.get("proposal_checksum")
            == PROPOSAL_CHECKSUM,
        "checkpoint_authorization":
            checkpoint.get("authorization_checksum")
            == AUTHORIZATION_CHECKSUM,
        "checkpoint_checksum":
    ...
    proposal_states = proposal.get("states") or []
    checkpoint_states = checkpoint.get("states") or []

    checks["deterministic_sequence"] = (
        [state.get("sequence") for state in manifest_states]
        == list(range(1, EXPECTED_STATES + 1))
        == [state.get("sequence") for state in proposal_states]
        == [state.get("sequence") for state in checkpoint_states]
    )
    ...
    checks["state_identity"] = all(
        ...
        and state.get("state")
            == manifest_by_sequence[sequence].get("state")
            == proposal_by_sequence[sequence].get("state")
        and str(state.get("state_lgd_code"))
            == str(
                manifest_by_sequence[sequence].get(
                    "state_lgd_code"
                )
            )
            == str(
                proposal_by_sequence[sequence].get(
                    "state_lgd_code"
                )
            )
        ...
        and state.get("ordered_row_manifest_sha256")
            == manifest_by_sequence[sequence].get(
        ...
        and state.get("source_file_sha256")
            == manifest_by_sequence[sequence].get("sha256")
            == proposal_by_sequence[sequence].get(
                "source_file_sha256"
            )
    ...
    if failed:
        raise ValueError(
            "REHABILITATION_ARTIFACT_VALIDATION_FAILED:"
            + ",".join(failed)
        )

    return manifest, proposal, authorization, checkpoint
...
def selected_rows(
    ...
    rows = connection.execute(text(r"""
        ...
        select
          ...
          input.canonical_block_lgd_code
...
def apply_batch(
        ...
        row_metadata = {
            **base_metadata,
            ...
            "runtime_geometry_sha256":
                row["runtime_geometry_sha256"],
            "name_disposition": row["disposition"],
        }
    ...
    review_rows = connection.execute(text(r"""
        ...
        set
          ...
          reviewer_decision =
            'ACCEPT_NORMALIZED_DIRECT_MATCH',
