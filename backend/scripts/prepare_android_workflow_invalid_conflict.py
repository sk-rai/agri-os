...
def prepare_stage_state(db, dry_run: bool) -> dict:
    baseline_status = ensure_android_baseline(db, dry_run)
    cycle = (
        db.query(CropCycle)
        .filter(CropCycle.id == CYCLE_ID, CropCycle.tenant_id == TENANT_ID)
        .first()
    ...
    if not cycle or not stage:
        ...
        raise RuntimeError(f"failed to create crop cycle/stage baseline for {CYCLE_ID}")
    ...
    return {"baseline": baseline_status, "before": before, "after": after}
