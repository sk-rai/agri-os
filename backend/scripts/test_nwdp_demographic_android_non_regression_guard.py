...
def main():
    ...
    check(counts["approved_for_promotion_count"] == 450026, "Four hundred fifty thousand twenty-six profiles remain approved after duplicate-blocked cleanup", counts)
    check(counts["active_promoted_profile_row_count"] == 450026, "Four hundred fifty thousand twenty-six profiles are promoted and active for web/admin verification", counts)
    check(counts["remaining_eligible_profile_row_count"] == 0, "No approved inactive profiles remain eligible after duplicate-blocked cleanup", counts)
