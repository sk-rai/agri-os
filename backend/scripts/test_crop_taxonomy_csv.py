...
def main():
    ...
    try:
        ...
        check(report["file_name"] == "taxonomy.csv", "valid taxonomy report includes uploaded file name")
        ...
        check(required_summary_keys.issubset(set(report["summary"].keys())), "valid taxonomy report exposes admin summary contract", sorted(report["summary"].keys()))
        ...
        check(applied["batch_id"] == report["batch_id"], "applied taxonomy response echoes batch id")
        required_applied_counts = {"created", "updated", "unchanged", "edges_created"}
        check(required_applied_counts.issubset(set(applied["report"]["applied_counts"].keys())), "applied taxonomy response exposes applied count contract", sorted(applied["report"]["applied_counts"].keys()))
