    ...
    with SessionLocal() as db:
        return dict(db.execute(text("""
            select
              ...
              count(*) filter (where review_status = 'APPROVED_FOR_PROMOTION') as approved_for_promotion_count,
              count(*) filter (where review_status = 'MANUAL_REVIEW') as manual_review_count
...
def assert_current_baseline_contract(client: TestClient, headers: dict, before: dict) -> None:
    ...
    check(summary["auto_candidate_count"] == before["auto_candidate_count"], "Summary auto candidate count matches current baseline", summary)
    check(summary["approved_for_promotion_count"] == before["approved_for_promotion_count"], "Summary approved count matches current baseline", summary)
    check(summary["manual_review_count"] == before["manual_review_count"], "Summary manual review count matches current baseline", summary)
    check(data["approved_vs_manual_review"] == {"approved_for_promotion_count": before["approved_for_promotion_count"], "manual_review_count": before["manual_review_count"]}, "Approved versus manual review summary matches current baseline", data["approved_vs_manual_review"])
