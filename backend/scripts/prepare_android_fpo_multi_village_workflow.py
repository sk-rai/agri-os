...
def reset_fixture(db, result: dict, dry_run: bool) -> None:
    ...
    deletes = [
        ...
        ("media_attachments", "delete from media_attachments where tenant_id = :tenant_id and (media_asset_id in (select id from media_assets where tenant_id = :tenant_id and (project_id = cast(:project_id as uuid) or farmer_id = any(cast(:farmer_ids as uuid[])))) or entity_id in (select id from broadcast_contents where tenant_id = :tenant_id and campaign_id in (select id from broadcast_campaigns where tenant_id = :tenant_id and project_id = cast(:project_id as uuid))))"),
        ("media_assets", "delete from media_assets where tenant_id = :tenant_id and (project_id = cast(:project_id as uuid) or farmer_id = any(cast(:farmer_ids as uuid[])))"),
        ...
        ("project_app_config_audit_events", "delete from project_app_config_audit_events where tenant_id = :tenant_id and project_id = cast(:project_id as uuid)"),
        ("field_event_reports", "delete from field_event_reports where tenant_id = :tenant_id and (project_id = cast(:project_id as uuid) or farmer_id = any(cast(:farmer_ids as uuid[])) or parcel_id = any(cast(:parcel_ids as uuid[])) or crop_cycle_id = any(cast(:cycle_ids as uuid[])))"),
