...
def main():
    ...
    try:
        ...
        project_filter_options_response = client.get(f"/api/v1/reports/projects/{project_id}/trace/filter-options")
        check(project_filter_options_response.status_code == 200, "project trace filter options returns 200", project_filter_options_response.text[:300])
        project_filter_options = project_filter_options_response.json()
        check(any(item["id"] == str(farmer_id) for item in project_filter_options["farmers"]), "project trace filter options include farmer")
        check(any(item["id"] == str(parcel_id) and item["farmer_id"] == str(farmer_id) for item in project_filter_options["parcels"]), "project trace filter options include parcel")
        check("RICE" in project_filter_options["crops"] and "KHARIF" in project_filter_options["seasons"], "project trace filter options include crop and season")
        check("ACTIVE" in project_filter_options["cycle_statuses"], "project trace filter options include cycle status")
        check(any(item["code"] == "TILLERING" for item in project_filter_options["stages"]), "project trace filter options include stage")
        check("FERTILIZER" in project_filter_options["activity_types"], "project trace filter options include activity type")
        check(any(item["code"] == "UREA_46_N" for item in project_filter_options["inputs"]), "project trace filter options include input")
        check(any(item["code"] == PRODUCT for item in project_filter_options["products"]), "project trace filter options include product")
        ...
        geometry_lookup_response = client.get(f"/api/v1/reports/lookup?project_id={project_id}&geometry_status=MISSING")
        check(geometry_lookup_response.status_code == 200, "lookup geometry status filter returns 200", geometry_lookup_response.text[:300])
        geometry_lookup = geometry_lookup_response.json()
        check(geometry_lookup["filters"]["project_id"] == str(project_id) and geometry_lookup["filters"]["geometry_status"] == "MISSING", "lookup echoes geometry filters")
        check(any(item["id"] == str(parcel_id) for item in geometry_lookup["parcels"]), "lookup geometry missing filter includes parcel")
        captured_lookup = client.get(f"/api/v1/reports/lookup?project_id={project_id}&geometry_status=CAPTURED").json()
        check(all(item["id"] != str(parcel_id) for item in captured_lookup["parcels"]), "lookup geometry captured filter excludes missing parcel")
        geometry_csv_response = client.get(f"/api/v1/reports/lookup.csv?project_id={project_id}&geometry_status=MISSING")
        check(geometry_csv_response.status_code == 200, "lookup geometry CSV returns 200", geometry_csv_response.text[:300])
        ...
        sync_gap_response = client.get(f"/api/v1/reports/sync-health?project_id={project_id}&gap_only=true")
        check(sync_gap_response.status_code == 200, "sync health gap filter returns 200", sync_gap_response.text[:300])
        sync_gap = sync_gap_response.json()
        check(sync_gap["filters"]["gap_only"] is True, "sync health gap filter is echoed")
        check(sync_gap["summary"]["event_count"] >= 1, "sync health gap filter returns gaps")
        check(all(item["materialized"] is False for item in sync_gap["recent_events"]), "sync health gap filter only returns unmaterialized rows")
        sync_csv_response = client.get(f"/api/v1/reports/sync-health.csv?project_id={project_id}&gap_only=true")
        check(sync_csv_response.status_code == 200, "sync health CSV returns 200", sync_csv_response.text[:300])
        check(sync_csv_response.headers["content-type"].startswith("text/csv"), "sync health CSV content type is text/csv")
        sync_csv_rows = list(csv.DictReader(io.StringIO(sync_csv_response.text)))
        check(any(row["entity_type"] == "PARCEL_GEOMETRY" and row["entity_id"] == str(parcel_id) for row in sync_csv_rows), "sync health CSV includes gap row")
        ...
        check(dashboard["schema_version"] == "admin_dashboard.v1", "dashboard schema version is correct")
        check(dashboard["summary"]["project_count"] >= 1, "dashboard includes project count")
        check(dashboard["summary"]["activity_count"] >= 1, "dashboard includes activity count")
        check(any(item["id"] == str(project_id) for item in dashboard["projects"]), "dashboard includes recent project")
        check(any(item["id"] == str(farmer_id) and item["trace_url"] == f"/farmer-trace/{farmer_id}" for item in dashboard["farmers"]), "dashboard includes recent farmer trace link")
        check(any(item["id"] == str(parcel_id) and item["trace_url"] == f"/parcel-trace/{parcel_id}" for item in dashboard["parcels"]), "dashboard includes recent parcel trace link")
        check(any(item["activity_id"] == str(activity.id) for item in dashboard["activities"]), "dashboard includes recent activity")
        check(any(item["crop_code"] == "RICE" for item in dashboard["crop_distribution"]), "dashboard includes crop distribution")
        check(any(item["activity_type"] == "FERTILIZER" for item in dashboard["activity_count_by_type"]), "dashboard includes activity type summary")
        project_dashboard_response = client.get(f"/api/v1/reports/admin-dashboard?project_id={project_id}")
        check(project_dashboard_response.status_code == 200, "project-scoped admin dashboard returns 200", project_dashboard_response.text[:300])
        project_dashboard = project_dashboard_response.json()
        check(project_dashboard["filters"]["project_id"] == str(project_id), "dashboard echoes project filter")
        check(project_dashboard["summary"]["project_count"] == 1 and project_dashboard["summary"]["activity_count"] == 1, "project dashboard summary is scoped")
        empty_date_dashboard = client.get(f"/api/v1/reports/admin-dashboard?project_id={project_id}&date_from=2027-07-21").json()
        check(empty_date_dashboard["summary"]["activity_count"] == 0 and empty_date_dashboard["activities"] == [], "dashboard date filter excludes activity")
        print("PASS")
    finally:
