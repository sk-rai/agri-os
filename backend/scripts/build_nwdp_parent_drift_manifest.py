#!/usr/bin/env python3
"""Build a checksum-pinned, read-only NWDP parent-drift manifest."""
...
from scripts.nwdp_parent_drift_selector import (
...
SCHEMA_VERSION = "nwdp_parent_drift_manifest.v1"

EVIDENCE_DIR = (
    PROJECT_ROOT
    / "data/staged/core_stack/promotion_review"
    / "20260924-geography-coverage-reconciliation-v1"
)
EVIDENCE_SUMMARY = EVIDENCE_DIR / "evidence_summary.json"
ACTIONABILITY_AUDIT = (
    EVIDENCE_DIR
    / "nwdp_parent_drift_actionability_audit.json"
)
SELECTOR_REVALIDATION = (
    EVIDENCE_DIR
    / "nwdp_parent_drift_selector_revalidation.json"
)
SELECTOR_SCRIPT = (
    BACKEND_ROOT
    / "scripts/nwdp_parent_drift_selector.py"
)

EVIDENCE_SUMMARY_SHA256 = (
    "1e7601b65dd533750c1bf5852be6d473"
    "3f416a6509ef5627882bddb30d09f83f"
)
ACTIONABILITY_AUDIT_SHA256 = (
    "855f04ffe069f0cd1ccfc62fa9bb54f"
    "68cbefe0849dbb6e35c1d58ddcf02dd13"
)
SELECTOR_REVALIDATION_SHA256 = (
    "0dfd14460f75e588a5ca9825486dc800"
    "8c7ac35006f12c0e31ef0d8ea3d7caf5"
)
SELECTOR_SCRIPT_SHA256 = (
    "9d637e58f61c0ce68b6b60baef0e9f4"
    "49d3e4ab2e65a59e6b3c899a0a881a1dd"
)

DEFAULT_OUTPUT = (
    EVIDENCE_DIR
    / "nwdp_parent_drift_manifest.json"
)
...
EXPECTED_REPAIRED_ROWS = {
    "f41d0f7d-84d0-5c03-bdfa-6cec72d0209b": {
        "state": "Haryana",
        "index": 3_982,
        "geometry_hash":
            "662d100b84ae2122ca2ab815f2471030"
            "f54b78f5becc69fd636b846ff0dd541e",
        "row_checksum":
            "318b9ce76000b10ae51c4ac84d862fd"
            "7059b4312b49fbb412975df0ef3b6d77a",
    },
    "7bbeca1d-09a4-5732-99cd-2b9d6e11f6e1": {
        "state": "Uttarakhand",
        "index": 16_709,
        "geometry_hash":
            "d0b2783759e6fcb9e70fe874cc473353"
            "691a820309fd500ca279e67312ca56ee",
        "row_checksum":
            "03f9610c6dedb5f836f0ed4ac015440c"
            "75ced4e7a94327a076f395ca9fdf5ced",
    },
}
...
def validate_pinned_evidence() -> None:
    ...
    actionability = json.loads(
        ACTIONABILITY_AUDIT.read_text(encoding="utf-8")
    )
    revalidation = json.loads(
        SELECTOR_REVALIDATION.read_text(encoding="utf-8")
    )
    ...
    checks = {
        ...
        "summary_no_writes":
            ...
            is False,
        "summary_rows":
            summary.get("nwdp", {}).get(
                "parent_drift_deterministic_rows"
            )
            == EXPECTED_ROWS,
        "summary_unauthorized":
            summary.get(
                "parent_drift_proposal_evidence",
                {},
            ).get("authorized") is False,
        "actionability_healthy":
            actionability.get("healthy") is True,
        "actionability_no_writes":
            actionability.get(
        ...
        "revalidation_checks":
            all(
                revalidation.get("checks", {}).values()
            ),
    }
