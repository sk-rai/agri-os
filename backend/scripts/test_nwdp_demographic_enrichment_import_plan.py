...
def main() -> int:
    ...
    check(data["planned_active_profile_rows"] == 0, "Plan keeps profiles inactive", data)
    check(data["planned_promoted_profile_rows"] == 0, "Plan keeps profiles not promoted", data)
    check(data["planned_review_status_counts"]["AUTO_CANDIDATE"] == 250, "Plan review bucket is auto candidate", data["planned_review_status_counts"])
    check(data["planned_promotion_status_counts"]["NOT_PROMOTED"] == 250, "Plan promotion bucket is not promoted", data["planned_promotion_status_counts"])
    check(data["planned_approved_vs_manual_review"] == {"approved_for_promotion_count": 0, "manual_review_count": 0}, "Plan exposes approved versus manual-review summary", data["planned_approved_vs_manual_review"])
    check(len(data["planned_state_district_summary"]) > 0, "Plan exposes state/district summary", data["planned_state_district_summary"][:3])
    first_district = data["planned_state_district_summary"][0]
    check("state_or_ut" in first_district, "State/district summary includes state", first_district)
    check("district" in first_district, "State/district summary includes district", first_district)
    check("planned_profile_rows" in first_district, "State/district summary includes planned profile rows", first_district)
    check("auto_candidate_count" in first_district, "State/district summary includes auto candidate count", first_district)
    ...
    check(sample["review_status"] == "AUTO_CANDIDATE", "Sample keeps review status as auto candidate", sample)
    check(sample["promotion_status"] == "NOT_PROMOTED", "Sample keeps promotion status as not promoted", sample)
    check(sample["is_active"] is False, "Sample remains inactive", sample)
