#!/usr/bin/env python3
"""Authorization-pinned generic state-batch validation metadata apply."""
...
SCHEMA_VERSION = (
    "boundary_geometry_validation_metadata_authorized_state_batch.v1"
)
...
APPROVED_FIRST_INDEX: int | None = None
APPROVED_LAST_INDEX: int | None = None
...
def authorization_checksum(
    authorization: dict[str, Any],
) -> str:
    state = authorization.get("state") or {}
    ...
    payload = {
        ...
        "national_plan_checksum":
            authorization.get("national_plan_checksum"),
        "state": {
            "state_slug": state.get("state_slug"),
            "state_or_ut": state.get("state_or_ut"),
            "import_batch_id": state.get("import_batch_id"),
            "source_sha256": state.get("source_sha256"),
            "batch_id": state.get("batch_id"),
            "plan_checksum": state.get("plan_checksum"),
            "selected_row_count":
                state.get("selected_row_count"),
            ...
            "rollback_token": state.get("rollback_token"),
        },
        "approval": {
        ...
        "permissions": {
            "apply_authorized":
                permissions.get("apply_authorized"),
            "rollback_authorized":
                permissions.get("rollback_authorized"),
            "maximum_row_count":
                permissions.get("maximum_row_count"),
            "validated_without_repair_only":
                permissions.get(
                    "validated_without_repair_only"
                ),
            "geometry_repair_allowed":
                permissions.get("geometry_repair_allowed"),
            "runtime_eligibility_change_allowed":
                permissions.get(
                    "runtime_eligibility_change_allowed"
                ),
            "candidate_write_allowed":
                permissions.get("candidate_write_allowed"),
            "runtime_table_write_allowed":
                permissions.get("runtime_table_write_allowed"),
...
def configure_authorization(
    ...
    *,
    ...
    global PLAN_CHECKSUM
    global IMPORT_BATCH_ID
    global STATE_SLUG
    global STATE_NAME
    global NATIONAL_PLAN_CHECKSUM
    global APPROVED_BATCH_ID
    global APPROVED_OPERATOR
    global APPROVED_APPROVER
    global APPROVAL_REFERENCE
    global APPROVED_ROLLBACK_TOKEN
    global APPROVED_ROW_COUNT
    global APPROVED_FIRST_INDEX
    global APPROVED_LAST_INDEX
    global AUTHORIZATION_CHECKSUM

    if authorization.get("schema_version") != AUTHORIZATION_SCHEMA:
        raise ValueError("AUTHORIZATION_SCHEMA_MISMATCH")
    if authorization.get("status") != "AUTHORIZED":
        raise ValueError("STATE_BATCH_NOT_AUTHORIZED")

    expected_checksum = authorization_checksum(authorization)
    if (
        authorization.get("authorization_checksum")
        != expected_checksum
    ):
        raise ValueError("AUTHORIZATION_CHECKSUM_MISMATCH")
    ...
    if any(state.get(key) in (None, "") for key in required_state_fields):
        raise ValueError("AUTHORIZATION_STATE_IDENTITY_INCOMPLETE")

    if not authorization.get("national_plan_checksum"):
        raise ValueError("NATIONAL_PLAN_CHECKSUM_REQUIRED")
    if not all(
        approval.get(key)
        for key in [
            "operator",
        ...
        raise ValueError("AUTHORIZATION_APPROVAL_INCOMPLETE")

    requested_permission = (
        "rollback_authorized" if rollback
        else "apply_authorized"
    )
    if permissions.get(requested_permission) is not True:
        raise ValueError(
            "ROLLBACK_NOT_AUTHORIZED"
            if rollback
            else "APPLY_NOT_AUTHORIZED"
        )

    if (
        int(permissions.get("maximum_row_count") or 0) < 1
        or int(permissions.get("maximum_row_count") or 0) > 500
        or int(state["selected_row_count"]) >
            int(permissions["maximum_row_count"])
    ):
        raise ValueError("AUTHORIZATION_ROW_LIMIT_INVALID")

    if permissions.get("validated_without_repair_only") is not True:
        raise ValueError("SAFE_CLASSIFICATION_POLICY_REQUIRED")
    ...
    if any(permissions.get(key) is not False for key in prohibited):
        raise ValueError("PROHIBITED_PERMISSION_ENABLED")
    ...
    NATIONAL_PLAN_CHECKSUM = authorization[
        "national_plan_checksum"
    ]
    APPROVED_BATCH_ID = state["batch_id"]
    APPROVED_OPERATOR = approval["operator"]
    APPROVED_APPROVER = approval["approver"]
    APPROVAL_REFERENCE = approval["approval_reference"]
    APPROVED_ROLLBACK_TOKEN = state["rollback_token"]
    APPROVED_ROW_COUNT = int(state["selected_row_count"])
    APPROVED_FIRST_INDEX = state.get(
        "first_source_feature_index"
    )
    APPROVED_LAST_INDEX = state.get(
        "last_source_feature_index"
    )
    AUTHORIZATION_CHECKSUM = expected_checksum


def validate_invocation_checksums(
    expected_authorization_checksum: str,
    expected_national_plan_checksum: str,
) -> None:
    if (
        expected_authorization_checksum
        != AUTHORIZATION_CHECKSUM
    ):
        raise ValueError(
    ...
    if (
        expected_national_plan_checksum
        != NATIONAL_PLAN_CHECKSUM
    ):
        raise ValueError(
...
def load_authorization(path: Path) -> dict[str, Any]:
    if not path.is_file():
        raise ValueError("AUTHORIZATION_JSON_NOT_FOUND")
    try:
        authorization = json.loads(
            path.read_text(encoding="utf-8")
        )
    except (OSError, json.JSONDecodeError) as exc:
        raise ValueError("AUTHORIZATION_JSON_INVALID") from exc
    if not isinstance(authorization, dict):
        raise ValueError("AUTHORIZATION_JSON_INVALID")
...
def validate(args: argparse.Namespace, plan: dict[str, Any]) -> None:
    ...
    if (
        plan["batch"].get("first_source_feature_index")
        != APPROVED_FIRST_INDEX
    ):
        raise ValueError("APPROVED_FIRST_INDEX_MISMATCH")
    if (
        plan["batch"].get("last_source_feature_index")
        != APPROVED_LAST_INDEX
    ):
...
def write_outputs(
    ...
    csv_path = output_dir / (
        "boundary_validation_metadata_authorized_state_batch_rows.csv"
    )
    audit["output_files"] = {
        "json": str(json_path),
        "csv": str(csv_path),
...
def main() -> int:
    ...
    try:
        ...
        validate_invocation_checksums(
            args.authorization_checksum,
        ...
        print(json.dumps({
            "schema_version": SCHEMA_VERSION,
            "healthy": False,
            "mode": "AUTHORIZED_STATE_BATCH_PREFLIGHT_REJECTED",
            "error": str(exc),
            "guardrails": {
    ...
    audit = {
        ...
        "national_plan_checksum": NATIONAL_PLAN_CHECKSUM,
