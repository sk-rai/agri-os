...
This is the Android handoff payload map. Android should treat backend schemas, feature flags, and response fields as source-of-truth.

The generated payload bundle lives in `docs/samples/android/` and is regenerated by:
...
| File | Android use |
| --- | --- |
| `01-mode-bootstrap.json` | Decide farmer/agent/dual-mode entry path after login. |
| `02-app-config-bootstrap.json` | Apply project-effective branding, feature flags, profile form hints, and runtime config. |
| `03-pin-code-villages.json` | Render PIN-code village candidates when available. |
| `04-form-farmer-registration.json` | Render farmer profile capture fields. |
| `05-form-parcel-registration.json` | Render parcel/land capture fields including location and geometry source. |
| `06-form-soil-profile.json` | Render soil profile capture fields. |
| `07-form-options.json` | Hydrate backend-owned option sets for forms. |
| `08-profile-contract.json` | Validate Android renderer assumptions for profile flows. |
| `09-farmer-create-response.json` | Map farmer create response into local model/cache. |
| `10-parcel-create-response.json` | Map parcel create response into local model/cache. |
| `11-soil-profile-create-response.json` | Map soil profile create response into local model/cache. |
| `12-profile-readiness.json` | Render backend-owned readiness and missing-field guidance. |
| `13-soil-enrichment-summary.json` | Render soil enrichment summary cards. |
| `14-soil-enrichment-latest-or-error.json` | Handle no-latest-soil-enrichment response. |
| `15-weather-latest-snapshot.json` | Render backend-owned weather card. |
| `16-broadcast-feed.json` | Render assigned farmer advisory feed. |
| `17-broadcast-detail.json` | Render advisory detail and media/deeplink fields. |
| `18-broadcast-read-response.json` | Update local state after read action. |
