...
### Remaining geometry work � deferred

The CoRE/LGD activation track is paused with sufficient promoted coverage for Android/backend integration testing: 20 active promoted districts / 60 active mapping rows across Karnataka, Maharashtra, and Punjab.

Remaining geometry work before expanding the two held Karnataka districts:

- review the held-low-margin packet generated by `backend/scripts/build_core_lgd_held_low_margin_review_artifact.py`;
- compare BharatAtlas LGD district geometry against CoRE overlay geometry for Chamarajanagar/Chamarajanagara (`29/531`) and Davanagere/Davangere (`29/535`);
- confirm whether the low-margin rows are real ecological/biogeographic transition-zone intersections or source-boundary artifacts;
- only after that confirmation, run the scoped guarded activation workflow district-by-district.

Until this review is resumed, the held rows stay `POLY_REV`, `APPROVED_FOR_PROMOTION`, and inactive. Their existing `LOCAL_DEMO_DISTRICT_FALLBACK` rows remain active.
