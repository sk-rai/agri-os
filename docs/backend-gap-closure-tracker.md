...
| Android DigiPin GPS materialization | Active next | Android Flow 39 passed parcel PIN_DROP coverage. Farmer home DigiPin follow-up contract now documents DTO/display/debug-smoke expectations for home_digipin=4P3JK852C9 from backend response, null-without-GPS handling, and no local Android computation. | `docs/android-digipin-gps-materialization-test.md`, `maestro/39-land-summary-digipin-debug-smoke.yaml`, `backend/scripts/test_digipin_farmer_parcel_fields.py` |
