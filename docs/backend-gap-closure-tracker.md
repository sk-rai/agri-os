...
| Land-intelligence summary cards | Active next | Runtime and admin API v1 added for informational Android-ready region/season/soil-water/main-crop/alternate-crop summary cards using existing `land_intelligence_summary_overrides`. Smoke verified default generation, tenant override, and deactivate fallback. Next: add admin screen and Playwright smoke, then Android handoff. | `backend/app/modules/master_data/api/land_intelligence_summary.py`, `backend/scripts/test_land_intelligence_summary_api.py`, `docs/admin-localization-and-land-intelligence-summary-plan.md` |
