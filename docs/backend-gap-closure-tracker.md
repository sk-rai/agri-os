...
| CoRE polygon export + LGD overlay | Active next | CoRE GEE exports staged and normalized; CoRE class metadata and district fallback are ready. Dry-run overlay is still blocked by missing LGD-compatible boundary geometry. | `backend/scripts/normalize_core_geojson_exports.py`, `backend/scripts/validate_core_lgd_overlay_inputs.py`, `docs/core-geojson-normalization.md` |
