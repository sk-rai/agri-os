...
| Admin localization overrides | Active next | Migration `053` added durable localization tables and seeded 308 platform-owned content keys across forms, options, and workflow stages. English fallback is complete and verifier is ready for the admin API contract. Next: read/list/apply override API and admin UI surface. | `backend/scripts/audit_admin_localization_content_sources.py`, `backend/scripts/seed_admin_localization_content_keys.py`, `backend/scripts/verify_admin_localization_tables.py`, `docs/admin-localization-and-land-intelligence-summary-plan.md` |
