...
| CoRE polygon export + LGD overlay | Active next | Survey of India ABDB source validation added. SOI is now preferred official geometry source for review: district layer has 808 records with STATE_LGD/DIST_LGD fields, but 31 invalid district LGD and 28 invalid state LGD rows require filtering. DB import remains blocked on SOI/BharatAtlas comparison and review/promotion workflow. | `backend/scripts/validate_survey_of_india_boundary_source.py`, `docs/survey-of-india-boundary-source-review.md`, `docs/core-lgd-manual-review-import-plan.md` |
