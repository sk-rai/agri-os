...
| CoRE polygon export + LGD overlay | Active next | Equal-area comparison added: dominant CoRE class is stable for all 2,355 district/region rows, with the same 68 low-overlap rows below 60%. DB import remains blocked on duplicate district-code review, low-overlap manual review, and MANUAL_REVIEW importer/precedence design. | `backend/scripts/compare_core_lgd_overlay_equal_area.py`, `docs/core-lgd-overlay-equal-area-review.md`, `docs/bharatlas-boundary-source-review.md` |
