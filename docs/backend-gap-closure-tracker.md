...
| CoRE polygon export + LGD overlay | Active next | Readiness audit, source checklists, and execution plan added. CoRE class metadata and district fallback are ready; polygon exports and LGD boundary geometry are missing. | `backend/scripts/audit_climate_polygon_overlay_readiness.py`, `backend/scripts/plan_core_polygon_lgd_overlay.py`, `docs/core-polygon-lgd-overlay-plan.md` |
