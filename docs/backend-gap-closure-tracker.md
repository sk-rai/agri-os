...
| CoRE polygon export + LGD overlay | Active next | District-level dry-run overlay candidates generated for 785 Bharatlas LGD districts ? 3 CoRE systems. Candidate files are manual-review artifacts; DB import is still blocked on source/method review. | `backend/scripts/generate_core_lgd_overlay_candidates.py`, `backend/scripts/verify_core_lgd_overlay_candidates.py`, `docs/core-lgd-overlay-candidate-generation.md` |
