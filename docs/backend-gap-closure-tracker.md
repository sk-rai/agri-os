...
| CoRE polygon export + LGD overlay | Active next | Readiness audit, source checklists, execution plan, and dry-run input validator added. CoRE class metadata and district fallback are ready; polygon exports and LGD boundary geometry are still missing locally. | `backend/scripts/audit_climate_polygon_overlay_readiness.py`, `backend/scripts/validate_core_lgd_overlay_inputs.py`, `docs/core-lgd-overlay-input-validation.md` |
