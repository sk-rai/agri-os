...
| Admin localization overrides | Active next | Admin localization API and `/localization` screen added for summary, search/list, tenant/project/language override upsert, deactivate, and effective fallback preview. Smoke test passed with authenticated admin token. Next: apply effective overrides into Android-facing backend-driven form/option/workflow payloads. | `backend/app/modules/master_data/api/localization.py`, `web/src/app/(admin)/localization/page.tsx`, `backend/scripts/test_admin_localization_api.py`, `docs/admin-localization-and-land-intelligence-summary-plan.md` |
