...
| Admin localization overrides | Active next | Read-only audit inventoried 468 backend-owned translatable content keys across forms, options, and workflow stages. English fallback is complete; Hindi is mostly present; Kannada/Marathi/Punjabi need native review. Next: table/API contract for `localized_content_keys` and `localized_content_overrides`. | `backend/scripts/audit_admin_localization_content_sources.py`, `docs/admin-localization-and-land-intelligence-summary-plan.md` |
