...
| CoRE polygon export + LGD overlay | Active next | Low-overlap review added. Results: 68 low-overlap rows bucketed as 38 ecological/climatic transition, 19 manual review, 9 coastal/island geometry, and 2 source-version drift. DB import remains blocked on MANUAL_REVIEW importer/precedence design and manual promotion policy. | `backend/scripts/review_core_lgd_low_overlap_rows.py`, `docs/core-lgd-low-overlap-review.md`, `docs/bharatlas-backend-lgd-district-crosswalk-review.md` |
