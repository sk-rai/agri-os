...
| Android DigiPin GPS materialization | Watch for farmer-side follow-up | Android Flow 39 passed parcel PIN_DROP coverage: Android sent centroid lat/lng, backend returned centroid_digipin=4P3JK852C9, Android displayed backend response, and emitted android_computed_digipin=false. Remaining follow-up: farmer home DigiPin DTO/display/hydration coverage. | `docs/android-digipin-gps-materialization-test.md`, `maestro/39-land-summary-digipin-debug-smoke.yaml`, `backend/scripts/test_digipin_utility.py`, `backend/scripts/test_digipin_farmer_parcel_fields.py`, `backend/scripts/test_sync_digipin_materialization.py` |
