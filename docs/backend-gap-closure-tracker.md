...
| Admin localization overrides | Watch during Android/admin QA | Admin localization API and `/localization` screen added; published tenant/project/language overrides now flow into Android-facing forms, option sets, bootstrap summaries, and profile hydration form contracts while preserving English fallback and backend-owned semantics. Next: browser smoke the admin screen and later extend the same pattern to workflow-stage/detail views and land-intelligence summaries. | `backend/app/modules/master_data/api/localization.py`, `backend/app/modules/master_data/localization_runtime.py`, `web/src/app/(admin)/localization/page.tsx`, `backend/scripts/test_android_localization_override_delivery.py` |
