...
| CoRE polygon export + LGD overlay | Active next | Survey of India ABDB source validation and district name/code alignment review added. SOI is an official geometry reference, but the current extracted district shapefile's DIST_LGD is not safe as a direct backend LGD key: only 2 rows match name+code cleanly and 565 rows point to different backend districts. BharatAtlas remains the preferred operational LGD-keyed geometry source for current CoRE overlay review; Inactive manual-review import completed for 2,298 polygon-derived CoRE/LGD candidate rows. Admin read-only review endpoint/page and smoke tests added for side-by-side fallback vs `POLY_REV` candidate review. No promotion controls exist yet and no rows are automatically promoted. | `backend/scripts/validate_survey_of_india_boundary_source.py`, `docs/survey-of-india-boundary-source-review.md`, `docs/core-lgd-manual-review-import-plan.md` |
