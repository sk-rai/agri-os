...
| Android FPO project closure migration notice | Active next | Added V1 backend contract/verifier for backend-triggered project-closure broadcast notice, farmer-visible continuation CTA, and enrollment completion transition to SELF_SERVICE without deleting farmer/parcel/crop-cycle data. | backend/scripts/verify_android_fpo_project_closure_migration_notice.py, docs/android-fpo-project-closure-migration-notice-test.md |
...
| Admin localization overrides | Watch during Android/admin QA | Admin localization API, `/localization` screen, Android-facing override delivery, and Playwright browser smoke are in place. UI smoke verified search, publish override, API effective-label cross-check, deactivate, and fallback restoration. Next: extend the same admin/editable payload pattern to land-intelligence summary cards. | `backend/app/modules/master_data/api/localization.py`, `backend/app/modules/master_data/localization_runtime.py`, `web/src/app/(admin)/localization/page.tsx`, `web/smoke/localization_admin_smoke.mjs`, `backend/scripts/test_android_localization_override_delivery.py` |
| Land-intelligence summary cards | Watch during Android/admin QA | Runtime API, admin override API, `/land-intelligence-summary` screen, and Playwright smoke are in place for informational Android-ready region/season/soil-water/main-crop/alternate-crop cards. Browser smoke verified generated default, save override, runtime API cross-check, deactivate, and fallback restoration. Next: Android informational card handoff. | `backend/app/modules/master_data/api/land_intelligence_summary.py`, `web/src/app/(admin)/land-intelligence-summary/page.tsx`, `web/smoke/land_intelligence_summary_admin_smoke.mjs`, `backend/scripts/test_land_intelligence_summary_api.py` |
