...
| CoRE polygon export + LGD overlay | Active next | BharatAtlas boundary source review added: acceptable for dry-run/manual-review candidates, not authoritative. DB import remains blocked on duplicate district-code review, low-overlap review, and area-method decision. | `backend/scripts/review_bharatlas_boundary_source.py`, `docs/bharatlas-boundary-source-review.md`, `docs/core-lgd-overlay-candidate-review.md` |
