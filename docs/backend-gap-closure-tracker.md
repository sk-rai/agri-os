...
| Android land-intelligence summary screen | Ready for Android | Android handoff contract added for backend-driven read-only region/season/weather/soil-water/main-crop/alternate-crop summary. V1 is informational only and must not block onboarding; V2 can add clickable details. | `docs/android-land-intelligence-summary-screen-test.md`, `backend/scripts/test_land_intelligence_summary_api.py`, `docs/admin-localization-and-land-intelligence-summary-plan.md` |
| Android DigiPin GPS materialization | Active next | Backend already computes canonical farmer home and parcel centroid DigiPin from GPS/centroid coordinates. Android now needs focused Maestro coverage proving capture lat/lon, backend-computed DigiPin display, null-without-GPS behavior, and hydration after reload. | `docs/android-digipin-gps-materialization-test.md`, `backend/scripts/test_digipin_utility.py`, `backend/scripts/test_digipin_farmer_parcel_fields.py`, `backend/scripts/test_sync_digipin_materialization.py` |
