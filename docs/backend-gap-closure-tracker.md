...
| Android agent-assisted farmer management | Watch during Android | Backend now enforces assigned-field-agent authority for farmer/parcel profile PATCH when `X-Actor-ID` is a known active field agent. Android PASS confirmed assigned update and unassigned `403`; fixture/verifier now also cover `>1` assigned farmers for primary agent worklist selection/search. | `backend/scripts/verify_android_agent_assisted_farmer_management.py`, `docs/android-agent-assisted-farmer-management-test.md` |
