...
| CoRE polygon export + LGD overlay | Active next | Backend/BharatAtlas LGD crosswalk audit added. Results: 716 exact matches, 50 name variants, 1 state-code mismatch, 16 BharatAtlas-only rows, 11 backend-only rows, no duplicate LGD district codes. DB import remains blocked on state mismatch review, source-version drift handling, low-overlap manual review, and MANUAL_REVIEW importer/precedence design. | `backend/scripts/audit_bharatlas_backend_lgd_district_crosswalk.py`, `docs/bharatlas-backend-lgd-district-crosswalk-review.md`, `docs/core-lgd-overlay-equal-area-review.md` |
