...
This artifact presents Andaman and Nicobar Islands bounded validation metadata
batch 1 for administrative review.

This document is an approval candidate only. It does not authorize an enabled
database apply.
...
| Final rollback token confirmation | `PENDING` |
| Pre-apply database baseline recapture | `PENDING` |
| Complete 500-row plan review | `PENDING` |
| Enabled implementation review | `PENDING` |
...
- ready for enabled bounded apply implementation review: no
- ready for real 500-row apply: no
...
No enabled apply may rely on this artifact until every pending administrative
field is resolved and a separately reviewed decision explicitly changes the
approval status.